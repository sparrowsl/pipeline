...
export async function POST({ request }) {
  ...
  const repoData = await getAllMarkdownFiles(owner, repo); // Fetch GitHub data
 ...
 */
async function getAllMarkdownFiles(owner, repo, path = '') {
  try {
    const headers = GITHUB_TOKEN ? { Authorization: `token ${GITHUB_TOKEN}` } : {};
    ...
    for (const item of response.data) {
      if (item.type === 'file' && item.name.endsWith('.md')) {
        console.log('Markdown File:', item.path);
        // If it's a markdown file, retrieve its content
        const fileContent = await getFileContent(owner, repo, item.path);
        markdownFiles.push({ path: item.path, content: fileContent });
      } else if (item.type === 'dir') {
        // If it's a directory, recursively get markdown files
        const filesInDir = await getAllMarkdownFiles(owner, repo, item.path);
        markdownFiles.push(...filesInDir);
      }
    }

    return markdownFiles;
  } catch (error) {
    console.error(`Error retrieving markdown files from ${path}:`, error);
    return [];
  }
}
