...
<form on:submit={handleSubmit} class="flex flex-col w-full text-base">
  ...
  <div class="flex flex-col w-full font-medium text-black whitespace-nowrap">
    ...
    <input
      ...
      id="name"
      bind:value={name}
      class="flex mt-3 w-full border border-black border-solid h-[40px] rounded-full px-4"
      required
    />
  </div>
  ...
  <button
    type="submit"
    class="self-stretch px-10 py-4 mt-8 w-full text-base font-light text-white bg-teal-900 h-[52px] rounded-full"
    disabled={loading}
  >
  ...
  <div class="flex flex-wrap items-center justify-between w-full gap-6 mt-6 text-sm leading-none">
    <label class="flex items-center gap-2 font-medium text-black">
      <input
        ...
        class="rounded-full border-2 border-black h-[18px] w-[18px]"
      />
      Remember me
    </label>
  </div>
